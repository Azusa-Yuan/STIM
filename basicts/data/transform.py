...
def standard_transform(data: np.array, output_dir: str, train_index: list, history_seq_len: int, future_seq_len: int, norm_each_channel: bool = False) -> np.array:
    ...
    if norm_each_channel:
        mean, std = data_train.mean(axis=(0, 1), keepdims=True), data_train.std(axis=(0, 1), keepdims=True)
    else:
