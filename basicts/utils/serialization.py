...
from .adjacent_matrix_norm import calculate_scaled_laplacian, calculate_symmetric_normalized_laplacian, calculate_symmetric_message_passing_adj, calculate_transition_matrix, asym_adj
...
def load_adj(file_path: str, adj_type: str):
    ...
    if adj_type == "scalap":
        ...
        adj = [calculate_symmetric_message_passing_adj(
            adj_mx).astype(np.float32).todense()]
    elif adj_type == "asymadj":
        adj = [asym_adj(adj_mx).astype(np.float32).todense()]
